...
function extensionCards ($d, $pd)
  ...
  if (isset($d["file"]) and file_exists($d["file"]))
    ...
    foreach ($dets["groups"] as $gnm => $ga)
        ...
        if (count($pc) > 1)
          {
          if (in_array($dets["groups"][$gnm]["card"], array("list", "list2")))
...
END;
 ...
 function buildList2Card ($la) {
    ...
    ob_start();      
    echo <<<END
<li>$la[stitle]$ltop$la[ptitle]$lbottom$la[comment]$checked</li>
END;
    $html = ob_get_contents();
    ob_end_clean(); // Don't send output to client
...
function startGroupHtml ($gnm, $comment, $card, $tbc)
  ...
  /*if (in_array($card, array("image")))
    ...
    {$html = "$gtop<ul>";}   
  else if (in_array($card, array("list2")))
    {$html = "$gtop<ul>";}              
